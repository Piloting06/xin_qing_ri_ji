...
// GET /moodboard — 最近7天城市心情榜（按记录数排序）
router.get('/moodboard', auth, (req, res) => {
  try {
    const since = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);
    const rows = db.prepare(
      `SELECT city, emotion_type, COUNT(*) as cnt FROM moods
       WHERE city IS NOT NULL AND city != '' AND date >= ?
       GROUP BY city, emotion_type ORDER BY city`
    ).all(since);
    const agg = {};
    for (const r of rows) {
      const c = agg[r.city] || (agg[r.city] = { city: r.city, total: 0, mood: 0, moodCnt: 0, top: {} });
      c.total += r.cnt;
      c.top[r.emotion_type] = (c.top[r.emotion_type] || 0) + r.cnt;
      if (r.cnt > c.moodCnt) { c.moodCnt = r.cnt; c.mood = r.emotion_type; }
    }
    const board = Object.values(agg)
      .sort((a, b) => b.total - a.total)
      .slice(0, 12)
      .map((x) => ({ city: x.city, count: x.total, top_mood: x.mood }));
    res.json({ board, since });
  } catch (e) {
    console.error('moodboard:', e.message);
