...
// 必须把请求方的真实 IP 传给 ip-api：不带 ip 参数时它解析的是本服务器出口 IP，
// 会把所有用户都定位到服务器机房所在城市。
router.get('/location', async (req, res) => {
  const unknown = { lat: null, lon: null, city: '未知' };
  try {
    const fwd = req.headers['x-forwarded-for'];
    let ip = (Array.isArray(fwd) ? fwd[0] : fwd) || req.headers['x-real-ip'] || req.socket.remoteAddress || '';
    ip = String(ip).split(',')[0].trim().replace(/^::ffff:/, '');
    // 内网/回环地址定位不到真实位置，直接返回未知，避免误定位到机房
    const isLan = !ip || ip === '::1' ||
      /^(10\.|127\.|192\.168\.|169\.254\.|172\.(1[6-9]|2\d|3[01])\.)/.test(ip);
    // ip-api 的 IP 必须放在路径里（/json/{ip}），查询参数里的 ip 会被忽略
    const url = 'http://ip-api.com/json/' + (isLan ? '' : encodeURIComponent(ip)) +
      '?fields=status,lat,lon,city,regionName,country&lang=zh';
    ...
    http.get(url, (resp) => {
      let d = ''; resp.on('data', c => d += c); resp.on('end', () => {
        ...
        } catch (_) { res.json(unknown); }
      });
    }).on('error', () => res.json(unknown));
  } catch (_) { res.json(unknown); }
});
